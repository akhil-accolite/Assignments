...
public class BookDAO {
	...
	public Book addtocart(cart cart) {
		jdbcTemplate.update("insert into checkout values(?,?);", cart.getCheckoutid(), cart.getBookid());
		return null;
	}
	
	public List<Book> getcart(){
		return jdbcTemplate.query("select * from book b , checkout c where b.id = c.bookid", (rs, i) -> {
			return new Book(rs.getString("id"), rs.getString("title"), rs.getString("author"),
					rs.getString("description"), rs.getDouble("price"));
		});
	}
